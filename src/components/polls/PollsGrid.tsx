...
export default function PollsGrid({ polls, categories }: PollsGridProps) {
  const [selectedCategory, setSelectedCategory] = useState<PollCategory>("All");
  ...
  const filteredPolls = useMemo(() => {
    return polls.filter((poll) => {
      ...
      const matchesSearch =
        ...
        poll.context.toLowerCase().includes(q) ||
        poll.category.toLowerCase().includes(q) ||
        poll.options.some((o) => o.label.toLowerCase().includes(q)) ||
        (poll.tags && poll.tags.some((t) => t.toLowerCase().includes(q)));
    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 pb-6 border-b border-surface-dim/60">
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-lg text-xs font-semibold transition-all duration-150 cursor-pointer ${
                  isActive
        ...
        <div className="relative w-full md:w-80 flex-shrink-0">
          ...
          <input
            type="text"
            placeholder="Search polls, questions, topics..."
            value={searchQuery}
      ...
      <div className="flex items-center justify-between mb-8">
        <h3 className="text-xl font-bold font-display text-on-surface flex items-center gap-2">
          ...
          <span>
            {selectedCategory === "All" ? "All Executive Community Polls" : `${selectedCategory} Polls`}
          </span>
      ...
      {filteredPolls.length > 0 ? (
        ...
        /* Empty State */
        <div className="text-center py-16 px-4 bg-surface-pure rounded-2xl border border-surface-dim/70 max-w-2xl mx-auto">
          ...
          <p className="text-sm text-text-muted max-w-sm mx-auto mb-6">
            We couldn't find any polls matching "{searchQuery}" in {selectedCategory}.
          </p>
          <button
            onClick={() => {
              setSelectedCategory("All");
